...
from .const import DOMAIN, URL_CARDS, URL_COVERS
...
async def _async_register_paths(hass: HomeAssistant) -> None:
    """Serve the cards, and the job cover images."""
    if hass.data.get(_PATHS_DONE):
        return

    cards_dir = pathlib.Path(__file__).parent / "frontend"
    covers_root = pathlib.Path(hass.config.path(DOMAIN))
    await hass.async_add_executor_job(
        lambda: covers_root.mkdir(parents=True, exist_ok=True)
    )
    ...
    try:
        await hass.http.async_register_static_paths(
            [
                ...
                StaticPathConfig(URL_COVERS, str(covers_root), False),
            ]
        )
...
@callback
def async_card_urls() -> tuple[str, ...]:
    """The URLs the cards are served from, for diagnostics."""
    return tuple(f"{URL_CARDS}/{name}" for name in CARD_FILES)
